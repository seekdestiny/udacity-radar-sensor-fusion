...
%Form a signal containing a 77Hz sinusoid of amplitude 0.7 and a 43 Hz
%sinusoid of amplitude 2.
S = 0.7*sin(2*pi*77*t) + 2*sin(2*pi*43*t);

%Corrupt the signal with noise using the "randn" - noise generation
%function
X = S + 2*randn(size(t));

%Plot the noisy signal in the time domain. And you will see it is difficult
...
% At the end we just compute the single-sided spectrum as we reject the
% mirror image.
P1 = P2(1:L/2+1);
